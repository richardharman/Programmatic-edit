...
class read_csv:
            ...
            for row in reader: #Goes through all the rows on the data from the CSV      
                for file in self.files: #reads files from the training dataset
                    ...
                    if row[0].split(',')[0].split('.')[0] == _file_name: #Compares the ID's from the dataset and CSV
                        ...
                        os.chdir('/Users/Mxolisi/Documents/LabWork/Folders_from_CSV_Scripts/dataset/Day_01')
                        
                        self.extract_ffmpeg(file, os.path.join(os.getcwd(), folder_name + "/_file_name"))
    ...
    def getImages_from_terminal(self, out_dir):
         ...
         #cmd = "python convert.py --indir /Users/Mxolisi/Documents/LabWork/Folders_from_CSV_Scripts/dataset/Day_01 --outdir {0} --crop 1080 --size 200 --rate 30".format(out_dir)
         #subprocess.Popen(cmd, shell = True)
         in_dir = '/Users/Mxolisi/Documents/LabWork/Folders_from_CSV_Scripts/dataset/Day_01'
         crop_size = '1080'
         out_size = '200'
         framerate = '30'
         
         print(os.path.join(os.getcwd(), out_dir))
         
         conv.Pass_arguments(in_dir, out_dir, crop_size, out_size, framerate)
    ...
    def extract_ffmpeg(self, file_in, file_out):
        cmd = "ffmpeg -i frame "+ file_in + " " + file_out
        
        subprocess.Popen(["ffmpeg", "-i", file_in, file_out +"%03d.jpg"])
